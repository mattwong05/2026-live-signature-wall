...
- `/admin`：管理页，可设置本机 IP、生成签名页二维码、上传背景图、调整背景图显示强度、清空签名、编辑大屏标题、维护宣誓文案列表
...
- 如果已在管理页设置背景图，大屏会在最底层显示该图片，并按后台设定的显示强度控制透明度
...
- 标题固定为“我承诺：”
- 手机端会从后台维护的宣誓文案列表中由用户手动选择 1 条；非手机端仍展示单条文案
...
- 移动端签名提交成功后，会在页面中央显示统一风格的 Toast，并伴随轻量彩带庆祝效果
